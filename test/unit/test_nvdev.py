...
from tinygrad.runtime.support.nv.ip import NV_FLCN, find_vbios_rom_offset
...
class TestNVLifecycle(unittest.TestCase):
  ...
  def test_usb_boot_sets_gpu_time_before_booter_load(self):
    ...
    self.assertEqual(events, ["ptimer", "booter"])
...
class TestNVPageTableEntry(unittest.TestCase):
  ...
  def test_system_memory_aperture_matches_snooping(self):
    entry = self.make_encoding_entry(4)
    unsnooped = entry.nvdev.pte_t.decode(entry._encode_entry(0x214000, aspace=AddrSpace.SYS, snooped=False))
    snooped = entry.nvdev.pte_t.decode(entry._encode_entry(0x214000, aspace=AddrSpace.SYS, snooped=True))
    self.assertEqual((unsnooped['aperture'], snooped['aperture']), (3, 2))

...
from tinygrad.runtime.support.nv.ip import NV_FLCN, NV_GSP, NVGspLifecycleState
from tinygrad.runtime.support.system import USBPCIDevice
from tinygrad.runtime.support.usb import CustomASM24Controller

class ByteView:
  def __init__(self, size): self.data, self.nbytes = bytearray(size), size
...
class TestNVGSPSRAMBoot(unittest.TestCase):
  ...
  @staticmethod
  def lifecycle_flcn(view, chip_id=0x170000a1):
    flcn = object.__new__(NV_FLCN)
    flcn.nvdev = types.SimpleNamespace(chip_id=chip_id, pci_dev=types.SimpleNamespace(gsp_lifecycle_view=lambda: (view, 0x0FF00000)))
    flcn._sb_image, flcn._booter_unload_image = b"FWSEC" * 100, b"BOOTER" * 80
    flcn.sb_code_off, flcn.sb_data_off = 1, 2
    flcn.sb_imem_pa, flcn.sb_imem_va, flcn.sb_imem_sz = 3, 4, 5
    flcn.sb_dmem_pa, flcn.sb_dmem_va, flcn.sb_dmem_sz = 6, 7, 8
    flcn.sb_pkc_off, flcn.sb_engid, flcn.sb_ucodeid = 9, 10, 11
    flcn.booter_unload_code_off, flcn.booter_unload_code_sz = 12, 13
    flcn.booter_unload_data_off, flcn.booter_unload_data_sz = 14, 15
    return flcn

  def test_lifecycle_record_round_trips_images_metadata_and_state(self):
    view = ByteView(1 << 20)
    writer = self.lifecycle_flcn(view)

    writer._store_lifecycle_record(NVGspLifecycleState.PREPARED)
    immutable_header = bytes(view[:NV_FLCN.LIFECYCLE_RECORD.size])
    writer.set_lifecycle_record_state(NVGspLifecycleState.PROCESS_READY)
    self.assertEqual(bytes(view[:NV_FLCN.LIFECYCLE_RECORD.size]), immutable_header)

    reader = object.__new__(NV_FLCN)
    reader.nvdev = writer.nvdev
    self.assertEqual(reader.attach_lifecycle_record(), NVGspLifecycleState.PROCESS_READY)
    self.assertEqual((reader.sb_image_paddr, reader.booter_unload_image_paddr),
                     (0x0FF01000, 0x0FF02000))
    self.assertEqual((reader.sb_code_off, reader.sb_data_off, reader.sb_ucodeid), (1, 2, 11))
    self.assertEqual((reader.booter_unload_code_off, reader.booter_unload_data_sz), (12, 15))

    reader.clear_lifecycle_record()
    self.assertEqual(bytes(view[:8]), bytes(8))

  def test_lifecycle_record_rejects_corrupt_image(self):
    view = ByteView(1 << 20)
    writer = self.lifecycle_flcn(view)
    writer._store_lifecycle_record(NVGspLifecycleState.RM_READY)
    view.data[0x1000] ^= 1

    reader = object.__new__(NV_FLCN)
    reader.nvdev = writer.nvdev
    with self.assertRaisesRegex(RuntimeError, "checksum mismatch"): reader.attach_lifecycle_record()

  def test_lifecycle_record_can_be_cleared_after_reset_without_attachment(self):
    view = ByteView(1 << 20)
    view[:len(NV_FLCN.LIFECYCLE_MAGIC)] = NV_FLCN.LIFECYCLE_MAGIC
    flcn = object.__new__(NV_FLCN)
    flcn.nvdev = self.lifecycle_flcn(view).nvdev

    flcn.clear_lifecycle_record(persisted=True)

    self.assertEqual(bytes(view[:len(NV_FLCN.LIFECYCLE_MAGIC)]), bytes(len(NV_FLCN.LIFECYCLE_MAGIC)))

  def test_lifecycle_state_update_validates_immutable_record(self):
    view = ByteView(1 << 20)
    writer = self.lifecycle_flcn(view)
    writer._store_lifecycle_record(NVGspLifecycleState.PREPARED)
    view.data[0x1000] ^= 1
    ...
    with self.assertRaisesRegex(RuntimeError, "checksum mismatch"):
      writer.set_lifecycle_record_state(NVGspLifecycleState.WPR_ACTIVE)

    state_off = NV_FLCN.LIFECYCLE_RECORD.size
    self.assertEqual(NV_FLCN.LIFECYCLE_STATE.unpack(bytes(view[state_off:state_off+4]))[0],
                     NV_FLCN.LIFECYCLE_STATE_MAGIC | int(NVGspLifecycleState.PREPARED))

  def test_lifecycle_record_invalidates_stale_state_before_publishing_header(self):
    view = ByteView(1 << 20)
    state_off = NV_FLCN.LIFECYCLE_RECORD.size
    view[state_off:state_off+4] = NV_FLCN.LIFECYCLE_STATE.pack(
      NV_FLCN.LIFECYCLE_STATE_MAGIC | int(NVGspLifecycleState.PROCESS_READY))
    writer = self.lifecycle_flcn(view)
    writer._write_lifecycle_state = MagicMock(side_effect=RuntimeError("interrupted state publish"))

    with self.assertRaisesRegex(RuntimeError, "interrupted state publish"):
      writer._store_lifecycle_record(NVGspLifecycleState.PREPARED)

    reader = object.__new__(NV_FLCN)
    reader.nvdev = writer.nvdev
    with self.assertRaisesRegex(RuntimeError, "Invalid NVIDIA teardown state word 0x0"):
      reader.attach_lifecycle_record()

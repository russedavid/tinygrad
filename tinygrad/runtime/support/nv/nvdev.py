...
from tinygrad.runtime.support.nv.ip import NV_FLCN, NV_FLCN_COT, NV_GSP, NVGspLifecycleState
...
class NVLifecyclePhase(enum.Enum):
  ...
  LIFECYCLE_RECORD_CLEAR = "lifecycle_record_clear"
...
class NVDev:
  def __init__(self, pci_dev:PCIDevice, initialize:bool=True):
  ...
  def initialize(self):
    ...
    if getattr(self.pci_dev, "gsp_full_teardown", False) and isinstance(self.flcn, NV_FLCN):
      ...
      self._run_phase(NVLifecyclePhase.WPR_SETUP, NVLifecycleState.WPR_ACTIVE, self._init_wpr)
    ...
    self._run_phase(NVLifecyclePhase.GSP_RM_INIT, NVLifecycleState.RM_READY, self._init_gsp_rm)
  ...
  def _init_wpr(self):
    ...
    self.flcn.init_wpr()
    self.flcn.set_lifecycle_record_state(NVGspLifecycleState.WPR_ACTIVE)

  def _boot_gsp(self):
    assert isinstance(self.flcn, NV_FLCN)
    self._set_gpu_time()
    self.flcn.boot_gsp()
    self.flcn.set_lifecycle_record_state(NVGspLifecycleState.GSP_RUNNING)
  ...
  def _init_gsp_rm(self):
    self.gsp.init_hw()
    if isinstance(self.flcn, NV_FLCN): self.flcn.set_lifecycle_record_state(NVGspLifecycleState.RM_READY)

  def mark_root_ready(self):
    if isinstance(self.flcn, NV_FLCN): self.flcn.set_lifecycle_record_state(NVGspLifecycleState.ROOT_READY)

  def mark_process_ready(self):
    if isinstance(self.flcn, NV_FLCN): self.flcn.set_lifecycle_record_state(NVGspLifecycleState.PROCESS_READY)
  ...
  def fini(self, client_root:int|None=None):
    ...
    if not getattr(self.pci_dev, "gsp_full_teardown", False):
      if self.lifecycle_state in (NVLifecycleState.GSP_RUNNING, NVLifecycleState.RM_READY, NVLifecycleState.READY,
                                   NVLifecycleState.PROCESS_READY):
    ...
    gsp_started = self.lifecycle_state in (NVLifecycleState.GSP_RUNNING, NVLifecycleState.RM_READY, NVLifecycleState.READY,
      NVLifecycleState.PROCESS_READY, NVLifecycleState.CLIENT_RELEASED, NVLifecycleState.GSP_SUSPENDED)
    rm_client_ready = self.lifecycle_state in (NVLifecycleState.READY, NVLifecycleState.PROCESS_READY)

    if client_root is not None and rm_client_ready:
      def release_client():
        self.gsp.rpc_rm_free(client_root, client=client_root)
        if isinstance(flcn, NV_FLCN): flcn.set_lifecycle_record_state(NVGspLifecycleState.ROOT_RELEASED)
      self._run_teardown_phase(NVLifecyclePhase.RM_CLIENT_RELEASE, NVLifecycleState.CLIENT_RELEASED,
                               release_client, errors)
    if gsp_started:
      def suspend_gsp():
        self.gsp.fini_hw()
        if isinstance(flcn, NV_FLCN): flcn.set_lifecycle_record_state(NVGspLifecycleState.GSP_SUSPENDED)
      self._run_teardown_phase(NVLifecyclePhase.GSP_SUSPEND, NVLifecycleState.GSP_SUSPENDED, suspend_gsp, errors)
    ...
    if wpr_active:
      if not isinstance(flcn, NV_FLCN):
        ...
        def shutdown_fwsec():
          flcn.shutdown_fwsec()
          flcn.set_lifecycle_record_state(NVGspLifecycleState.FWSEC_COMPLETE)
        self._run_teardown_phase(NVLifecyclePhase.FWSEC_SHUTDOWN, NVLifecycleState.FWSEC_COMPLETE, shutdown_fwsec, errors)
    ...
    if not wpr_active:
      if isinstance(flcn, NV_FLCN):
        try: flcn.clear_lifecycle_record()
        except Exception as exc:
          errors.append(NVLifecycleError(NVLifecyclePhase.LIFECYCLE_RECORD_CLEAR, self.lifecycle_state, self._lifecycle_snapshot(), exc))
      self.lifecycle_state, self.lifecycle_phase = NVLifecycleState.CLOSED, None
  ...
  def _early_ip_init(self):
    ...
    if self.reg("NV_PFB_PRI_MMU_WPR2_ADDR_HI").read() != 0:
      if getattr(self.pci_dev, "gsp_flr_recovery", False): self._unclean_recovery_required = True
        ...
        raise RuntimeError("NVIDIA USB device has active WPR2 state without recoverable lifecycle data; "
  ...
  def _early_mmu_init(self):
    ...
    bar1_size = self.pci_dev.bar_info(1)[1] if getattr(self.pci_dev, "boot_mem_in_vram", False) else None
    lifecycle_size = getattr(self.pci_dev, "gsp_lifecycle_size", 0)
    if bar1_size is not None and not 0 <= lifecycle_size < bar1_size:
      raise ValueError(f"NVIDIA lifecycle reservation {lifecycle_size:#x} does not fit BAR1 size {bar1_size:#x}")
    cpu_visible_limit = bar1_size - lifecycle_size if bar1_size is not None else None
    self.mm = NVMemoryManager(self, self.vram_size - (64 << 20), boot_size=(2 << 20), pt_t=NVPageTableEntry, va_bits=bits, va_shifts=shifts,
      ...
      cpu_visible_limit=cpu_visible_limit, pa_start=bar1_size)

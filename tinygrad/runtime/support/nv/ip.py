...
def find_vbios_rom_offset(read32:Callable[[int], int]) -> int:
  signature = read32(0) & 0xffffffff
  if signature & 0xffff == 0xaa55: return 0

  fixed1 = read32(4)
  match (fixed1 >> 8) & 0xff:
    case 1 | 2: rom_offset = read32(((fixed1 >> 16) & 0x7fff) + 4)
    case 3:
      flash_status_offset = read32(read32(8) & 0xfffff)
      directory_offset = flash_status_offset + 0x1000
      rom_offset = read32(directory_offset + 8)
  return rom_offset
...
class NV_FLCN(NV_IP):
  ...
  def prep_ucode(self):
    vbios_bytes = memoryview(bytes(array.array('I', self.nvdev.mmio[0x00300000//4:(0x00300000+0x100000)//4])))
    try: vbios_off = rom_offset = find_vbios_rom_offset(lambda off: int.from_bytes(vbios_bytes[off:off+4], "little"))

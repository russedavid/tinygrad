...
- USB and PCI identities, SuperSpeed link, firmware protocol revision, and BAR
  setup.
...
### Controller firmware reproducibility

The physically qualified controller image reported protocol 1.0, revision 3,
and capabilities `0x7f`. Its recorded build identity is:

- firmware source revision `f3e65c52d73a5cdeeab764b090db4bef96902bc2`
- firmware branch name `nv-usb3-protocol-v1`
- wrapped image size 47,704 bytes
- wrapped image SHA-256
  `92ce01276727fb5862950d13818c253b80ca2bd169bd2c15a4a19f787fa82ffc`

This source revision is not reachable from the public firmware repository refs
in the clone audited here. Public `master` at
`821f218f4749da1ad29e56d855fd80a82baef590` also does not implement the `TG24`
`F4` information record required by the NVIDIA runtime. A fresh checkout of
firmware `master` therefore must not be described as a reproducible substitute
for the qualified image. Revision 3 also lacks reliable SRAM producer rearming
for the repeated F2 writes used by GSP streaming. Publishing and qualifying
revision 4 remains required for repeatable NVIDIA startup.
...
| Firmware compatibility check | Uses required commands but has no `F4` revision/capability gate | Requires protocol 1.x, revision 3+, and required capabilities; chained SRAM streaming requires revision 8 | NVIDIA stricter; AMD gate missing |
| Reproducible controller firmware | Public firmware source provides the AMD path | Revision 8 is physically qualified, but its source is not yet on a public firmware ref | NVIDIA source publication missing |
...
| Boot allocations | Reserved 32 MiB VRAM region survives partial boot | Boot data uses CPU-visible VRAM; the top 1 MiB of BAR1 stores teardown data and a lifecycle marker | Implemented |
...
| Clean process close | Remove GFX/SDMA queues, lower clocks, write clean marker | Free RM client, suspend GSP, run FWSEC shutdown and Booter Unload, verify WPR2 clear, clear lifecycle marker | Implemented and physically qualified |
...
| Unclean-exit recovery | Reject partial boot; use mode-1 reset when PSP/SMU are alive, then fully initialize | Require advertised PCIe FLR, issue one FLR, verify GFW and WPR2, clear the marker, then fully initialize | Implemented and physically qualified on RTX 3090 |
...
clear the lifecycle marker. The next process performs an ordinary fresh boot.

If a process dies while WPR2 is active, the next process does not reconnect to
the abandoned GSP queues or trust the persisted teardown record. It instead:
...
5. Clears the persisted lifecycle marker and continues with a normal fresh boot.
...
lifecycle-marker, and PCIe AER checks. Remaining lifecycle work is broader GPU
model qualification and publication of the revision-8 controller firmware
source used by the physical gate.
